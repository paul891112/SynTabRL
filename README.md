...
## Load Datasets and Perform Experiments
...
The privacy_result folder contains all experiment result of approaches generated by applying this proposed framework. To rerun the experiment, provide the training configuration file, approach, and pipeline flags as input to scripts/rlagent.py and run the program with training, sampling and model evaluation. After the completed data generation pipeline, evaluation results  are saved in SynTabRL_eval.json (or custom file name) in the base directory as specified in the config file. Modifying the input arguments to rerun other experiments.

```bash
conda activate tddpm

# Sum-based approach
python scripts/rlagent.py --config privacy_result/churn2/ddpm_cb_best_dynamic_adaptive_both/config.toml --adaptive_approach --train --sample --eval


# DCR-based approach
python scripts/rlagent.py --config privacy_result/churn2/ddpm_cb_best_dynamic_adaptive_single_metric/config.toml --adaptive_single_metric dcr --train --sample --eval
```

Note that when running experiments with single metric approaches, the training results are saved under a neutral name. After two immediate calls to train with different single-metric approaches, the results can be overwritten if no manual renaming took place in between. 
...
- `privacy_result/[ds_name]/` This directory contains training result generated by using the proposed framework on a specific dataset.
- `privacy_result/[ds_name]/ddpm_[cb|mlp]_best\_[approach_name]/` is the base directory name of a dataset using [approach_name] to train and [cb|mlp] to evaluate machine learning efficiency.
